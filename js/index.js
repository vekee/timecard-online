...
function createPDF(downLoadFlag = true) {
    ...
    if (downLoadFlag) {
        if ((/android|webos|iphone|ipad|ipod|blackberry|iemobile|opera mini/i.test(navigator.userAgent.toLowerCase()))) {
            alert("モバイル端末でPDF閲覧のみを対応しているため、メール送信よりPDFを取得してください。")
            // window.open(doc.output('bloburi', { filename: timeCardPDFName }), '_blank', 'location=yes')
            var pdfjsLib = window['pdfjs-dist/build/pdf'];
            pdfjsLib.GlobalWorkerOptions.workerSrc = '//mozilla.github.io/pdf.js/build/pdf.worker.js';
            var loadingTask = pdfjsLib.getDocument(doc.output('bloburi', { filename: timeCardPDFName }));
